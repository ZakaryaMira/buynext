  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] p-6">
      ...
      <div className="flex gap-6 items-start relative">
        ...
        <div className="sticky top-28 h-[calc(100vh-11rem)] overflow-y-auto scroll-mb-8 h-100">
        <OurCategories/>
        </div>

        <main className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.map((product) => (
            <AllProducts key={product.id} product={product}/> /* key react prop to truck list items */
          ))}
        </main>
